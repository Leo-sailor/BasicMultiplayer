...
@app.post("/{game_num}/join/{name}")
async def join_game(game_num: int, name: str):
...
@app.delete("/{game_num}/leave/{token}")
async def leave_game(game_num: int, token: int):
    if 0 <= game_num < len(games):
        try:
...
@app.put("/{game_num}/update/{token}/{velx},{vely}/{posx},{posy}")
async def update_game(game_num: int, token: int, velx: float, vely: float, posx: float, posy: float):
    if 0 <= game_num < len(games):
        try:
...
@app.get("/{game_num}/get/{excluded_names}")
async def get_game_state(game_num: int, excluded_names: str | None = None):
