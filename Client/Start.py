...
import pygame as pg
import requests
from BaseClasses import Vector2,display_text,IP,PORT
def start(screen:pg.Surface,*args)->tuple[Literal["Quit","Play"],list[int]]:
    running = True
    screen.fill((0, 0, 0))
    selected = 0
    display_text(50,"Pinging all servers...",screen,(255,255,255),"Arial",top_left_pos=Vector2(0,0))
    pg.display.flip()
    response = requests.get(f"http://{IP}:{PORT}/games")
